...
function useOpenClose(hoursData: Hours, timeZone = "") {
  ...
  const getIntervalOnDate = (hoursData: Hours, key: string, index: number) => {
    ...
    if (
      hoursData &&
      hoursData[key as DayStringType] &&
      hoursData[key as DayStringType]?.openIntervals
    ) {
      return {
        interval: hoursData[key as DayStringType]?.openIntervals,
        isHoliday: false,
      };
    } else {
      return null;
  ...
  const sortByDay = (week: Hours, timeZone: string): Hours => {
    const tmp = [];
    ...
    tmp.forEach((obj) => {
      orderedWeek[obj.key as DayStringType] = obj.value;
    });
